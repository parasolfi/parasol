...
const links = [
  { label: 'Get cover', to: '/cover' },
  { label: 'My cover', to: '/policies' },
]
  ...
  >
    <nav class="mx-auto flex max-w-6xl items-center justify-between gap-6 px-6 py-5">
      <NuxtLink to="/" class="flex items-center gap-2.5">
        <AppLogo />
        <span class="font-display text-xl tracking-tight text-ink">free</span>
      </NuxtLink>

      <ul class="hidden items-center gap-8 text-sm text-ink/60 md:flex">
        <li v-for="link in links" :key="link.to">
          <NuxtLink :to="link.to" class="transition-colors hover:text-ink">{{ link.label }}</NuxtLink>
        </li>
      </ul>
    ...
    >
      <li v-for="link in links" :key="link.to" class="py-2">
        <NuxtLink :to="link.to" @click="menuOpen = false">{{ link.label }}</NuxtLink>
      </li>
    </ul>
  </header>
</template>
